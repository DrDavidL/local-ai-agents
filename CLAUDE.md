...
Scheduled AI agents running on a home Mac via Ollama (local LLM). Four agents monitor literature, email, news, and grants, then deliver summaries via text, email, and notifications through the existing Shortcuts Bridge infrastructure. Model configurable in `config/agents.yaml` (currently `devstral-small-2`).
